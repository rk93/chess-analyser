...
function terminalCp(fen){try{const c=new Chess(fen);if(c.isCheckmate())return c.turn()==='w'?-10000:10000;if(c.isDraw()||c.isStalemate()||c.isInsufficientMaterial()||c.isThreefoldRepetition())return 0}catch{}return null}
function winProb(cp){const pawns=Math.max(-10,Math.min(10,(Number(cp)||0)/100));return 1/(1+Math.exp(-1.25*pawns))}
function moveLoss(before,after,ply){const wb=winProb(before),wa=winProb(after);return Math.max(0,ply%2===1?wb-wa:wa-wb)}
function basic(loss){if(loss<=.004)return'Best';if(loss<=.014)return'Excellent';if(loss<=.04)return'Good';if(loss<=.09)return'Inaccuracy';if(loss<=.18)return'Mistake';return'Blunder'}
...
function classify(i,ctx,data){const ply=i+1,before=data.evals[i],after=data.evals[i+1],loss=moveLoss(before,after,ply),played=ctx.ucis[i],best=data.bestMoves?.[i]||'',verified=!!data.positionExact?.[i]&&!!data.positionExact?.[i+1],san=sans()[i]||'',matched=!!best&&best===played,gain=moverGain(before,after,ply);if(!verified)return basic(loss);if(ply<=10&&loss<=.04)return'Book';if(matched&&san.endsWith('#'))return'Best';if(matched&&sacrificeSequence(i,ctx,data))return'Brilliant';if(matched&&loss<=.004&&gain>=150)return'Brilliant';if(matched&&loss<=.008&&gain>=70)return'Great';if(matched)return'Best';return basic(loss)}
function recalc(data,ctx){for(let i=0;i<ctx.positions.length;i++){const t=terminalCp(ctx.positions[i]);if(t!==null){data.evals[i]=t;data.positionExact[i]=true;if(i<data.bestMoves.length)data.bestMoves[i]=''}}const losses=[],labels=[],verified=[],wl=[],bl=[];for(let i=0;i<ctx.ucis.length;i++){const ply=i+1,l=moveLoss(data.evals[i],data.evals[i+1],ply),v=!!data.positionExact?.[i]&&!!data.positionExact?.[i+1];losses.push(l);verified.push(v);labels.push(classify(i,ctx,data));(ply%2?wl:bl).push(l)}data.losses=losses;data.labels=labels;data.moveVerified=verified;data.whiteAccuracy=accuracy(wl);data.blackAccuracy=accuracy(bl);data.correctionVersion=1;return data}
...
function refreshMoveTags(data){const els=moveEls();els.forEach((el,i)=>{const tag=el.querySelector('.moveTag');if(tag){tag.className='moveTag tag-'+String(data.labels[i]||'Good').toLowerCase();tag.textContent=data.labels[i]||'Good'}const practice=el.querySelector('.practiceMove');if(practice&&!(data.moveVerified[i]&&(data.labels[i]==='Mistake'||data.labels[i]==='Blunder')))practice.remove()})}
